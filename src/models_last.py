...
class TextureFlowModel(BaseModel):
    ...
    def build_model(self):
        ...
        # flow model with true input smooth
            ...
            self.t_dis = MultiDiscriminator(**self.dis_param2)
    ...
    def update_texture(self, inputs, lbps, maps):
        ...
        for i in range(len(fake_labels)):
            ...
            dis_fake_loss = self.adversarial_loss(fake_labels[i], False, True)
            dis_loss += (dis_real_loss + dis_fake_loss) / 2
        self.texture_adv_dis_loss = dis_loss / len(fake_labels)

        self.texture_adv_dis_loss.backward()
        self.t_dis_opt.step()
        if self.t_dis_scheduler is not None:
            self.t_dis_scheduler.step()

        dis_gen_loss = 0
        fake_labels = self.t_dis(outputs)
        for i in range(len(fake_labels)):
            dis_fake_loss = self.adversarial_loss(fake_labels[i], True, False)
            dis_gen_loss += dis_fake_loss
        self.texture_adv_gen_loss = dis_gen_loss / len(fake_labels) * 3
        self.texture_l1_loss = self.l1_loss(outputs, lbps) * 4
        self.texture_gen_loss = self.texture_l1_loss + self.texture_adv_gen_loss

        self.texture_gen_loss.backward()
        self.t_gen_opt.step()
        if self.t_gen_scheduler is not None:
            self.t_gen_scheduler.step()

        logs = [
            ("l_t_adv_dis", self.texture_adv_dis_loss.item()),
            ("l_t_l1", self.texture_l1_loss.item()),
            ("l_t_adv_gen", self.texture_adv_gen_loss.item()),
