...
class FID():
    ...
    samples respectivly.
    See --help to see fu rther details.
...
class Reconstruction_Metrics():
    ...
    def __init__(self, metric_list=['ssim', 'psnr', 'l1', 'mae', 'l2'], data_range=None, win_size=51, multichannel=True):
        self.data_range = data_range
        self.win_size = win_size
        self.multichannel = multichannel
        for metric in metric_list:
            if metric in ['ssim', 'psnr', 'l1', 'mae', 'l2']:
    ...
    def __call__(self, inputs, gts):
        ...
        if hasattr(self, 'l2'):
            l2_value = compare_mse(inputs, gts)
            result['l2'] = l2_value
    ...
    def calculate_from_disk(self, inputs, gts, save_path=None, debug=0):
        ...
        for index in range(len(input_image_list)):
            ...
            l2.append(compare_mse(img_gt, img_pred))
        ...
        print(
            ...
            "l2: %.4f" % round(np.mean(l2), 4),
            "l2 Variance: %.4f" % round(np.var(l2), 4),
        )    
